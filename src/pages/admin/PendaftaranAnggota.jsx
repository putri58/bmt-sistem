...
import {
  Search, Eye, CheckCircle, XCircle, X, Clock,
  UserCheck, UserX, Calendar, Filter, FileText, ZoomIn,
} from "lucide-react";
...
const statusIcons = { Menunggu: Clock, Disetujui: UserCheck, Ditolak: UserX };
...
function DocPreview({ label, path }) {
  ...
  const url = path ? `http://localhost:8000/storage/${path}` : null;
  ...
  return (
    <>
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        {isImage ? (
          <div className="group relative cursor-zoom-in overflow-hidden bg-slate-100" style={{ height: 160 }} onClick={() => setLightbox(true)}>
      ...
      {lightbox && isImage && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 p-4" onClick={() => setLightbox(false)}>
          <div className="relative max-h-[90vh] max-w-4xl" onClick={(e) => e.stopPropagation()}>
            <img src={url} alt={label} className="max-h-[85vh] max-w-full rounded-xl object-contain shadow-2xl" />
...
export default function PendaftaranAnggota() {
  ...
  async function fetchPendaftar() {
    try {
      ...
      console.error('Gagal ambil data pendaftar', err);
    } finally {
      setLoading(false);
    }
  }
  ...
  const filtered = pendaftarList.filter((p) => {
    const matchSearch =
      p.nama_anggota?.toLowerCase().includes(search.toLowerCase()) ||
      p.pendaftar_id?.toLowerCase().includes(search.toLowerCase()) ||
      p.no_identitas?.includes(search);
    const matchStatus = filterStatus ? p.status_pendaftaran === filterStatus : true;
  ...
  async function handleSetujui() {
    ...
    try {
      await api.post(`/pendaftar/${selected.id}/setujui`);
      await fetchPendaftar();
      closeModal();
    } catch (err) {
      alert('Gagal menyetujui pendaftar');
    } finally {
  ...
  async function handleTolak() {
    setActionLoading(true);
    try {
      await api.post(`/pendaftar/${selected.id}/tolak`, { alasan: alasanTolak });
      await fetchPendaftar();
      closeModal();
    } catch (err) {
  ...
  const counts = {
    Menunggu:  pendaftarList.filter((p) => p.status_pendaftaran === "Menunggu").length,
    Disetujui: pendaftarList.filter((p) => p.status_pendaftaran === "Disetujui").length,
    Ditolak:   pendaftarList.filter((p) => p.status_pendaftaran === "Ditolak").length,
  };
  ...
  return (
    <div>
      ...
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-slate-500">Tinjau dan proses setiap pengajuan pendaftaran anggota baru.</p>
      </div>
      ...
      <div className="mb-6 grid grid-cols-3 gap-4">
        <div className="rounded-2xl border border-orange-100 bg-orange-50 p-5">
          ...
          {counts.Menunggu > 0 && <p className="mt-3 text-xs text-orange-500">⚠ Perlu segera diverifikasi</p>}
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                <tr><td colSpan={7} className="py-14 text-center text-slate-400">Tidak ada data pendaftaran.</td></tr>
              ) : (
                filtered.map((row) => {
                  const StatusIcon = statusIcons[row.status_pendaftaran] || Clock;
                  return (
                    <tr key={row.id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-5 py-4 font-mono text-xs text-slate-500">{row.pendaftar_id}</td>
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-slate-400">{row.jenis_kelamin} · {row.kota}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-5 py-4">
                        <p className="font-mono text-xs text-slate-600">{row.no_identitas}</p>
                        <p className="text-xs text-slate-400">{row.jenis_identitas}</p>
                      </td>
                      <td className="px-5 py-4">
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-1.5 text-xs text-slate-500">
                          <Calendar size={12} />
                          {formatDate(row.tgl_daftar)}
                        </div>
                      </td>
                      <td className="px-5 py-4">
                        <span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[11px] font-semibold ${statusColors[row.status_pendaftaran]}`}>
                          <StatusIcon size={11} />
                          {row.status_pendaftaran}
                        </span>
                        {row.status_pendaftaran === "Disetujui" && row.id_anggota && (
                          <p className="mt-1 text-[10px] text-emerald-600">→ {row.id_anggota}</p>
                        )}
                      </td>
                      <td className="px-5 py-4">
                        <div className="flex items-center justify-center gap-2">
                          <button onClick={() => { setSelected(row); setModalMode("view"); }}
                            className="rounded-lg p-1.5 text-slate-400 hover:bg-blue-50 hover:text-blue-600 transition-colors" title="Lihat Detail">
                            <Eye size={15} />
                          </button>
                          {row.status_pendaftaran === "Menunggu" && (
                            <>
                              <button onClick={() => { setSelected(row); setModalMode("setujui"); }}
                                className="rounded-lg p-1.5 text-slate-400 hover:bg-emerald-50 hover:text-emerald-600 transition-colors" title="Setujui">
                                <CheckCircle size={15} />
                              </button>
                              <button onClick={() => { setSelected(row); setModalMode("tolak"); }}
      ...
      {modalMode === "view" && selected && (
        <Modal title="Detail Pendaftaran" onClose={closeModal} size="lg">
          <div className="mb-5 flex items-center gap-4">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-800">{selected.nama_anggota}</h3>
              <span className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-0.5 text-[11px] font-semibold ${statusColors[selected.status_pendaftaran]}`}>
          ...
          <div className="rounded-xl border border-slate-100 bg-slate-50 p-4 mb-4">
            ...
            <DetailRow label="ID Pendaftaran"    value={selected.pendaftar_id} />
            <DetailRow label="Nama Lengkap"      value={selected.nama_anggota} />
            <DetailRow label="No. Identitas"     value={`${selected.no_identitas} (${selected.jenis_identitas})`} />
            <DetailRow label="Tempat Lahir"      value={selected.tempat_lahir} />
            <DetailRow label="Tanggal Lahir"     value={formatTanggalLahir(selected.tanggal_lahir)} />
            <DetailRow label="Kewarganegaraan"   value={selected.kewarganegaraan} />
            <DetailRow label="Status Pernikahan" value={selected.status_perkawinan} />
            <DetailRow label="Jenis Kelamin"     value={selected.jenis_kelamin} />
            <DetailRow label="Nama Ibu Kandung"  value={selected.nama_ibu_kandung} />
          </div>

          <div className="rounded-xl border border-slate-100 bg-slate-50 p-4 mb-4">
            <p className="mb-2 text-xs font-bold uppercase tracking-wider text-slate-400">Alamat & Kontak</p>
            <DetailRow label="Alamat Rumah" value={selected.alamat_rumah} />
            <DetailRow label="Kota"         value={selected.kota} />
            <DetailRow label="Kode Pos"     value={selected.kode_pos} />
            <DetailRow label="Email"        value={selected.email} />
            <DetailRow label="No. Telp/HP"  value={selected.no_telp} />
          </div>
          ...
          <div className="rounded-xl border border-slate-100 bg-slate-50 p-4 mb-5">
            <p className="mb-2 text-xs font-bold uppercase tracking-wider text-slate-400">Info Pendaftaran</p>
            <DetailRow label="Tgl Pendaftaran" value={formatDate(selected.tgl_daftar)} />
            {selected.status_pendaftaran === "Disetujui" && (
              <DetailRow label="ID Anggota" value={selected.id_anggota} />
            )}
            {selected.status_pendaftaran === "Ditolak" && selected.alasan_tolak && (
              <DetailRow label="Alasan Tolak" value={selected.alasan_tolak} />
            )}
          </div>

          {selected.status_pendaftaran === "Menunggu" && (
      ...
      {modalMode === "setujui" && selected && (
        <Modal title="Konfirmasi Persetujuan" onClose={closeModal}>
          <div className="mb-5 flex items-start gap-4 rounded-2xl border border-emerald-100 bg-emerald-50 p-4">
            ...
            <div>
              <p className="font-semibold text-slate-800">{selected.nama_anggota}</p>
              <p className="text-xs text-slate-500">{selected.no_identitas} · {selected.jenis_identitas}</p>
              <p className="text-xs text-slate-500">{selected.email}</p>
            </div>
          </div>
          <p className="text-sm text-slate-600 leading-relaxed">
            Dengan menyetujui pendaftaran ini, <strong>{selected.nama_anggota}</strong> akan
            resmi menjadi anggota aktif KSPPS BMT Al Ittihad.
          </p>
      ...
      {modalMode === "tolak" && selected && (
        <Modal title="Konfirmasi Penolakan" onClose={closeModal}>
          ...
          <p className="mb-4 text-sm text-slate-600">Tolak pendaftaran dari <strong>{selected.nama_anggota}</strong>?</p>
          <div>
            <label className="mb-1.5 block text-xs font-semibold text-slate-600">
              Alasan Penolakan <span className="font-normal text-slate-400">(opsional)</span>
            </label>
            <textarea value={alasanTolak} onChange={(e) => setAlasanTolak(e.target.value)} rows={4}
              placeholder="Contoh: Dokumen identitas tidak valid..."
          ...
          <div className="mt-6 flex gap-3">
            ...
            <button onClick={handleTolak} disabled={actionLoading}
              className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-red-600 py-2.5 text-sm font-semibold text-white hover:bg-red-700 transition-colors disabled:opacity-60">
              <XCircle size={15} />
